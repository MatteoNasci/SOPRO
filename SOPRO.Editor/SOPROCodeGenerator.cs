...
namespace SOPRO.Editor
{
    ...
    public class SOPROCodeGenerator : ScriptableObject
    {
        ...
        void OnValidate()
        {
            ...
            Queue<int> indexes = new Queue<int>(4);
            ...
            for (int i = 0; i < types.Length; i++)
            {
                ...
                if (res.Length > 0)
                {
                    if (res.Contains(" "))
                        res = string.Empty;
                    else if (char.IsNumber(res[0]))
                        res = string.Empty;
                    else if (res.Any(c => !char.IsLetter(c) && !char.IsNumber(c)))
                        res = string.Empty;
                }
                if (res.Length == 0)
                {
